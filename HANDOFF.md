...
- **On-brand glass cursors.** The site uses a custom cursor set drawn to match the brand (blue / silver, reflective glass): a glossy arrow with a chrome-silver top highlight, a navy glass edge, and a soft drop shadow so it reads on both the light sky and the dark navy areas. Two NEW files in `public/`: `cursor.png` (the default arrow, ~27x36, hotspot near the tip at `4 4`) and `cursor-link.png` (a brighter "lit" version). `global.css` sets `html{ cursor: url('/cursor.png') 4 4, auto; }`, and a rule at the very end of the file gives every clickable (`a, button, summary, [role="button"], .metal, .rt-btn, .gthumb__x`) the `cursor-link.png` variant in place of the OS hand, while `input, textarea, [contenteditable]` keep a normal text caret. The Zeffy iframe is a separate document so its cursor is unaffected, and touch devices have no cursor at all. (This replaced an earlier supplied arrow image that did not fit the look; if `public/cursor.png` was already uploaded with the old image, re-upload it to overwrite, the file name is the same.)
