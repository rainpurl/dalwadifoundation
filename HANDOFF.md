...
- **Team headshots are uploaded, not linked.** In the staff portal's About tile, each team member's photo is now a file picker instead of a URL field. On selection the image is converted in the browser (`toAvif`: AVIF where supported, then WebP, then a resized JPEG) and shrunk to ~400px, and the result is embedded straight into the saved content as a data URL in `team[i].photo`. No new endpoint or blob storage is needed: `about.astro` and `live.ts` already render `team[i].photo` through `<img src>`, which works for data URLs and for any old `http` URLs still in the data. A small circular preview and a Remove button sit beside the picker. Code lives in `renderTeam` (`portal.ts`), the new `blobToDataURL` helper, and `toAvif` (now takes an optional `maxDim`, and gained a resized-JPEG fallback so a headshot can never embed a full-size original). Caveat: headshots ride inside `content.json` (fetched each load, cached in localStorage), so they are kept small; for a large team a dedicated blob endpoint like the gallery's would be the next step.
...
> ⚠️ **This zip is AHEAD of the GitHub repo** (twelve batches of changes). Upload these changed files on github.com and let Cloudflare rebuild. **No file deletions are needed** (unlike the WebGL revert). Files touched across the batches: `astro.config.mjs`, `src/styles/global.css`, `src/layouts/Base.astro`, `src/layouts/Page.astro`, `src/pages/index.astro`, `src/pages/about.astro`, `src/pages/contribute.astro`, `src/pages/staff/index.astro`, `src/components/Foundation.astro`, `src/scripts/stage.ts`, `src/scripts/metal.ts`, `src/scripts/live.ts`, `src/scripts/portal.ts`, `src/components/Towers.astro`, `src/data/pillars.ts`, `src/data/site.ts`, `functions/_lib/defaults.js`, `functions/_lib/kv.js`, `functions/api/content.js`, and `HANDOFF.md`. **NEW files (Batch 12, glass cursors):** `public/cursor.png` and `public/cursor-link.png` (drop both in the existing `public/` folder alongside `favicon.png`/`logo.png`). **NEW files (Batch 5, Official Documents):** `functions/api/docs.js` and `functions/api/docs/[id].js` (the `docs/` subfolder is new). **NEW files (Batch 7):** `src/components/ZeffyFrame.astro`, `functions/api/gallery.js`, and `functions/api/gallery/[id].js` (the `gallery/` subfolder under `functions/api/` is new, so create it when uploading the nested `[id].js`).
