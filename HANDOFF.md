...
- **Each pillar has an "Our impact" button.** The partner links are now silver (`.metal .metal--sm`) and the impact button is the blue one (`.metal .metal--dark .metal--sm`), in `.tp__actions` in `Towers.astro`. Clicking it opens an impact "subpage": `stage.ts` adds `.is-impact` to `.app`, flies the other three columns out to the sides (`.fly-l` / `.fly-r`), centers the chosen column, and reveals a `#impact` overlay with that pillar's title, body, and a spread of cards that animate in with a stagger. While it is open the site title (`.wordmark`) shrinks up to the very top, and the close control is a slightly larger blue back button (`.metal .metal--dark .metal--back`) in the top-left corner (not a silver close x). Close via that back button, Escape, or navigating away. The exact choreography (the chosen pillar is currently mostly covered by a near-opaque overlay so the cards stay readable) is the part most worth tuning live; the data and editing are solid.
