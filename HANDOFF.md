...
- **On-brand glass cursors (arrowhead).** The site uses a custom cursor set drawn to match the brand (blue / silver, reflective glass): a sharp **arrowhead / dart** pointing up-left, with a chrome-silver top highlight, a navy glass edge, and a soft drop shadow so it reads on both the light sky and the dark navy areas. Two files in `public/`: `cursor.png` (the default, ~36x36, hotspot at the tip `4 4`) and `cursor-link.png` (a brighter "lit" version). `global.css` sets `html{ cursor: url('/cursor.png') 4 4, auto; }`, and a rule at the very end of the file gives every clickable (`a, button, summary, [role="button"], .metal, .rt-btn, .gthumb__x`) the `cursor-link.png` variant in place of the OS hand, while `input, textarea, [contenteditable]` keep a normal text caret. To redraw or reshape the cursor, edit `PTS_U` (the dart polygon) and the palette in the generator and re-export both PNGs at ~28px tall.
