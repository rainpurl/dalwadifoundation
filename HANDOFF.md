...
- **On-brand glass cursors (arrowhead).** The site uses a custom cursor set drawn to match the brand (blue / silver, reflective glass): a sharp, **notched arrowhead** pointing up-left, fairly upright (~30 degrees off vertical), with a moderate back-notch (tuned between a flat triangle and a deep barb), with a chrome-silver top highlight, a navy glass edge, and a soft drop shadow so it reads on both the light sky and the dark navy areas. Two files in `public/`: `cursor.png` (the default, ~25x30, hotspot at the tip `3 3`) and `cursor-link.png` (a brighter "lit" version). `global.css` sets `html{ cursor: url('/cursor.png') 3 3, auto; }`, and a rule near the end of the file gives every clickable (`a, button, summary, [role="button"], .metal, .rt-btn, .gthumb__x`) the `cursor-link.png` variant in place of the OS hand, while `input, textarea, [contenteditable]` keep a normal text caret. To reshape it, edit `CANON` (the upright arrowhead polygon), `TILT` (degrees off vertical), and the palette in the cursor generator, then re-export both PNGs at ~24px tall.
- **Partner-link hover/click colours.** Each pillar's partner button(s) turn a brand colour on hover and click while keeping the metallic shine (only the gradient, plus the text colour where the fill is dark, change; the bevel / shimmer / lift are untouched). House of Devi -> warm beige, charity: water -> #ffd53b (yellow), Hilton College -> #c8102e (red, white text), Daya -> #8a1553 (deep magenta, white text), HAWC -> #6c2c91 (purple, white text). Rules live in `global.css` just after the `.metal` block, keyed by `.tower[data-pillar="..."] .tp__actions a.metal:first-of-type` (plus `:nth-of-type(2)` for HAWC, safety's second link).
- **Team headshots are uploaded, not linked (stored like gallery photos).** In the staff portal's About tile, each team member's photo is a file picker (not a URL field). On selection the image is converted in the browser (`toAvif`: AVIF where supported, then WebP, then a resized JPEG, shrunk to ~400px), uploaded to KV via `POST /api/headshot`, and the returned small URL `/api/headshot/<id>` is stored in `team[i].photo`. The About page renders it through `<img src>` (works for these URLs and any old `http`/data URLs still in the data). This mirrors the photo gallery's blob approach: an earlier version embedded the image as a data URL inside the content, which broke on the live site, so it was moved to blob storage and the content now holds only a tiny URL. A small circular preview and a Remove button sit beside the picker. Code: `renderTeam` + `uploadHeadshot` in `portal.ts`; `toAvif` (optional `maxDim`, a resized-JPEG fallback, and an `onFail` callback the uploader uses to reject formats the browser cannot read, e.g. iPhone **HEIC**, with a clear message). Backend: `functions/api/headshot.js` (staff-only POST), `functions/api/headshot/[id].js` (public GET, cached hard), and `putHeadshotBlob`/`getHeadshot`/`deleteHeadshotBlob` in `kv.js` (blob in `hs:<id>`, content-type in KV metadata). Note: replacing or removing a headshot leaves the old blob orphaned in KV (small, harmless); a cleanup pass could be added later.
- **Staff portal uses the system cursor.** Per request, the glass cursor is disabled on `/staff`. `src/pages/staff/index.astro` carries a `<style is:global>` block (`html{ cursor: auto !important }` plus `cursor: pointer` for clickables) that Astro emits only on the staff route, so the portal shows the normal OS arrow/hand and the public pages are unaffected.
- **Zeffy donation frame keeps its own cursor (cannot be changed).** The embed is a cross-origin `<iframe>` served from Zeffy's own domain; browser security forbids the parent site from styling anything inside it, including the cursor. So over the donation form the visitor sees Zeffy's default cursor, not the glass arrowhead. There is no fix that preserves a working form (an overlay to "paint" a cursor on top would block clicks from reaching the iframe). Touch devices have no cursor at all, so this is desktop-only and cosmetic.
...
- **Hover/tilt now works on Chromium laptops.** Chrome and Edge on a touchscreen laptop report touch as the *primary* pointer, so the old `(hover: hover) and (pointer: fine)` test was false and the site fell back to tap-to-open with no hover and no cursor-follow (it looked like a half-mobile view). `stage.ts` now uses `(any-hover: hover) and (any-pointer: fine)`, which is true whenever a mouse or touchpad is present even alongside a touchscreen, so laptops get the full desktop behaviour in every browser. Firefox was unaffected because it already reported the mouse as primary.
- **Tap-to-open is decoupled from hover capability (follow-up fix).** The broader `any-hover/any-pointer` test above is also true on iPads and some phones (they advertise stylus/trackpad support), which wrongly classified those touch devices as hover-desktops, so the tap handler (gated on `!hoverCapable`) never attached and tapping a pillar did nothing, the centered modal card never appeared. `stage.ts` now derives a separate `touchCapable` from `navigator.maxTouchPoints` / `ontouchstart`, and attaches the tap handler when `touchCapable || !hoverCapable || reduce`. So every touch device gets tap-to-open (and the modal when `isPhone()`), reduced-motion users (who skip the hover-tilt pass) can still open pillars, and a hover-only desktop is the only configuration that stays hover-only, exactly as before. The hover-tilt pass is unchanged (`hoverCapable && !reduce`); a touchscreen laptop simply gets both.
- **Loads are faster and the old/default version no longer flashes.** A small boot script in both layout heads (`Base.astro`, `Page.astro`) runs once on a full load: it applies the cached font before first paint (so return visitors never see the Cormorant default flash) and kicks off the `/api/content` request immediately so it overlaps the JS bundle download. `live.ts` now caches the fetched content in `localStorage` (`dalwadi-content`) and, on every page load and in-site navigation, applies the cached copy instantly (correct font + copy with no wait) and then refreshes from the network in the background, updating and re-caching if anything changed. The content API stays `no-store`, so edits remain fresh; the cache just removes the visible gap. Net effect: repeat visits and navigation are instant and never show defaults; a brand-new device's first-ever visit still waits on the network (now started earlier), and if the font was changed away from Cormorant, only that first-ever visit briefly shows Cormorant before the fetch lands. Baking the chosen font as the default, or moving the pages to server-side rendering, would remove even that last gap.

**Batch 10 - per-pillar Impact subpage, Announcements bar, studio credit, page copyright:**

- **Each pillar has an "Our impact" button.** The partner links are now silver (`.metal .metal--sm`) and the impact button is the blue one (`.metal .metal--dark .metal--sm`), in `.tp__actions` in `Towers.astro`. Clicking it opens an impact "subpage": `stage.ts` adds `.is-impact` to `.app`, flies the other three columns out to the sides (`.fly-l` / `.fly-r`), centers the chosen column, and reveals a `#impact` overlay with that pillar's title, body, and a spread of cards that animate in with a stagger. The "Our impact" button sits slightly larger than the other small buttons (`.impact-btn` bumps the `metal--sm` size). While the subpage is open the site title (`.wordmark`) is hidden (it could overlap the pillar title), and the close control is a slightly larger blue back button (`.metal .metal--dark .metal--back`) in the top-left corner (not a silver close x). Close via that back button, Escape, or navigating away.
- **The Support page shows a horizontal carousel of the number cards.** `live.ts` (`buildImpactStrip`) mirrors every pillar's number cards (text cards are skipped) into `#impact-strip` at the bottom of the Support page, smaller and all the same fixed height, rendered twice for a seamless loop. It scrolls at the same speed as the About gallery by sharing the `--gallery-dur` variable (so `loadGallery` now also runs on Support, just to set that speed). The Zeffy iframe was shortened to make room (116px on desktop), and the strip is hidden on phones (<=640px) so the donation form keeps its space. The exact choreography (the chosen pillar is currently mostly covered by a near-opaque overlay so the cards stay readable) is the part most worth tuning live; the data and editing are solid.
- **Impact cards are editable per pillar in the Pillars tile.** Each pillar block now has an "Impact cards" section with two kinds of card: number cards (a stat plus a label, e.g. "12" / "Communities with clean water") and text cards (a paragraph). Add via "+ Number card" / "+ Text card", remove per row. Stored as `pillar.impact` (an array of `{stat,label}` or `{text}`). Defaults seeded in `pillars.ts` and `defaults.js` with placeholder numbers (edit before launch). The cards are baked hidden per column (`.tower__impact`), rebuilt by `live.ts` from saved data, and cloned into the overlay on open.
- **Announcements bar.** A new "Announcements" tile (text + optional link) saves to `settings.announcement = {text, href}`. `live.ts` fills `#announce`, a fixed bar across the very top edge of the homepage (hidden when the text is empty; the whole bar links out when a URL is given). Hidden automatically while the impact subpage is open.
- **Studio credit.** A subtle, link-styled-as-text line "built by STUDIO KATRESAI" sits at the very bottom of the staff portal, linking to https://katr.es (muted, no underline until hover).
- **Page copyright.** "(c) Dalwadi Foundation [year]" at the bottom of the About page (normal flow, above the navbar) and the Support page (a fixed strip on the dark background just above the navbar; the Zeffy iframe was shortened by 30px to make room). The year is baked at build and refreshed to the runtime year by `live.ts` (`.cr-year`), so it stays correct without a rebuild.

**Why the content API needed no change:** impact lives inside `pillars`, the announcement inside `settings`, and About sections inside `about`, all of which `functions/api/content.js` already passes through wholesale.

**Batch 9 - gallery refinements, custom About sections with rich text:**

- **Galleries are centered in the side whitespace** (not flush to the edges): each column's left/right offset is `calc((100vw - 720px) / 4 - var(--gallery-w) / 2)`, i.e. midway between the screen edge and the 720px centered copy. They stay hidden at and below 1200px, so they never appear on phones or tablets.
- **Carousel content is standardized at 30 tiles (15 per side) and loops seamlessly.** `live.ts` now caps real photos at 30, splits them across the two sides (so the same photo never shows on both), and pads each side to 15 with silver/blue filler tiles (`.gph`); tiles are shuffled so photos and filler intersperse. Each side renders its 15-tile set twice and scrolls `translateY(0 -> -50%)`, which loops forever with no seam. Because 15 tiles is taller than the viewport, a photo is never on screen at the same time as its duplicate (the old visible-duplicate problem is gone). Photos are spaced further apart (bigger `gap`). NOTE: with few or no photos the columns are mostly/all filler squares by design; add photos to replace them.
- **Staff can add custom rich text sections to the About page.** The About tile has a "Custom sections" manager: each section has an optional heading and a contenteditable body with a small toolbar (Bold, Italic, Underline, Link, Unlink). Formatting uses `document.execCommand` with `styleWithCSS` off, so it produces tags (`<b> <i> <u> <a>`), not inline styles. Sections are stored in `about.sections` (the content API already passes the whole `about` object through, so no API change was needed) and rendered on the About page after the intro by `live.ts`. Rendering is sanitized through an **inert DOMParser** whitelist (keeps only `b/strong/i/em/u/a/p/br/span`, drops every attribute except an http(s)/mailto `href`, and forces `target=_blank rel=noopener` on links); DOMParser is used specifically because it does not run scripts or fire image/onerror loads while parsing untrusted HTML. Sections have no baked default, so like the team list they appear once `live.ts` runs.

**Batch 8 - pillar label wrapping, adjustable carousel speed:**

- **Long pillar labels wrap to two lines.** `.tower__name` lost `white-space: nowrap` and gained `max-width: calc(var(--tw) - 14px)` with `text-align: center`, `line-height: 1.12`, and `overflow-wrap: break-word`, so a label that is too wide wraps within the tower face instead of overflowing. The mobile rule (the rotated vertical label, <=700px) explicitly keeps `white-space: nowrap` and `max-width: none`, since wrapping vertical text would look wrong.
- **Carousel scroll speed is staff-adjustable, with a faster default.** The side galleries scrolled too slowly. The track duration is now `var(--gallery-dur, 55s)` (was a fixed 85s; the right side is still 1.2x), so the default is already faster, and it can be tuned without code changes. The value (seconds per loop, lower is faster, clamped 10-180) lives in KV `gallery-speed`. `/api/gallery` GET now also returns `speed`; a POST with a JSON `{ "speed": N }` body and no file sets it (staff only, alongside the existing multipart photo upload). `live.ts` reads it and sets `--gallery-dur` on the page. The About tile has a "Carousel scroll speed" slider (15 to 130, left is faster) that saves on release and is independent of the About Save button (like the photo add/remove). Tuning still means a quick look at the live About page, but no longer needs a code change.

**Batch 7 - live donation preload, navbar back-slide fix, photo gallery:**

- **The donation form is now a single persistent iframe, preloaded live.** Prefetch alone still left a delay, so `ZeffyFrame.astro` renders one `<iframe transition:persist="zeffydonateframe">` that is included by BOTH layouts (so it survives every in-site navigation instead of reloading). It loads once in the background, hidden, and is shown only on the Support page via `html:has(.embed-wrap) #zeffy-frame` (the Support page now renders just an empty `.embed-wrap` background, no iframe of its own). Result: opening Support shows the already-loaded form with no buffer. It is intentionally NOT rendered on `/staff` (guarded by `Astro.url.pathname`). TRADE-OFF the user accepted: because the form is loaded on every public page, Zeffy may count those as form views. If that ever matters, the fallback is the lighter prefetch-only approach from Batch 6. The Batch 6 preconnect/dns-prefetch/prefetch tags are kept (they still warm the very first load).
- **Navbar back-slide fixed.** Home to subpage was already static; subpage to home, the bar slid away and snapped back. Cause: the incoming homepage HTML carries `.app.is-intro`, whose rule translates the navbar off-screen (`transform: translateY(100%)`), and the slide snapshot was taken with that state still applied (stage.ts only clears it on `astro:page-load`, which is after the snapshot). Fix: in the `astro:before-swap` handler (both layouts), if the incoming document has `#app.is-intro`, swap it to `no-splash is-settled is-revealed` BEFORE the snapshot, so the navbar is already in its resting position and the `sitenav` group stays put. Fresh loads of the homepage are unaffected (no before-swap), so the splash still plays normally.
- **Photo gallery on the About page.** Two vertical strips of slowly scrolling, slightly tilted photos in the left/right whitespace (`.gallery--left/--right` in `about.astro`, filled at runtime by `live.ts` from `/api/gallery`). Each photo gets a random tilt up to 30 degrees (a CSS `--rot` var, generated once per photo and reused for its duplicate so the looping scroll is seamless), rounded corners, and auto width. The set is rendered twice and scrolled with `@keyframes gallery-scroll` (translateY 0 to -50%); the two sides use different durations so they do not march in lockstep. Hovering a photo pauses only that side (`.gallery__track:has(.gimg:hover)`) and scales the photo up; `html{ overflow-x: clip }` keeps the expand from adding a horizontal scrollbar. The strips are hidden below 1200px (no room) and respect reduced-motion. Photos are split half to each side, so a full 30 shows 15 per side.
  - Staff add/remove them from the **About tile** (a Photo gallery section in that sheet): a thumbnail grid with per-photo remove, a count (x / 30), and a multi-file picker. Up to 30 photos, 5 MB each, enforced on both client and server. The browser re-encodes each photo to AVIF before upload (`canvas.toBlob('image/avif')`), falling back to WebP then the original file if the browser cannot encode AVIF (Chromium can; Safari/Firefox fall back), and large images are scaled down to 1600px max. Uploads run one at a time and changes save immediately (independent of the About Save button).
  - Storage and routes mirror the docs feature: metadata in KV `gallery`, bytes in `gimg:<id>`. `functions/api/gallery.js` = public GET (list, for the About page), staff POST (multipart upload) and DELETE (`?id=`). `functions/api/gallery/[id].js` = public GET serving the image bytes with a long immutable cache. These image URLs are public on purpose (the About page is public); docs downloads remain staff-only.
...
> ⚠️ **This zip is AHEAD of the GitHub repo** (twelve batches of changes). Upload these changed files on github.com and let Cloudflare rebuild. **No file deletions are needed** (unlike the WebGL revert). Files touched across the batches: `astro.config.mjs`, `src/styles/global.css`, `src/layouts/Base.astro`, `src/layouts/Page.astro`, `src/pages/index.astro`, `src/pages/about.astro`, `src/pages/contribute.astro`, `src/pages/staff/index.astro`, `src/components/Foundation.astro`, `src/scripts/stage.ts`, `src/scripts/metal.ts`, `src/scripts/live.ts`, `src/scripts/portal.ts`, `src/components/Towers.astro`, `src/data/pillars.ts`, `src/data/site.ts`, `functions/_lib/defaults.js`, `functions/_lib/kv.js`, `functions/api/content.js`, and `HANDOFF.md`. **NEW files (Batch 12, glass cursors):** `public/cursor.png` and `public/cursor-link.png` (drop both in the existing `public/` folder alongside `favicon.png`/`logo.png`). **NEW files (team headshot upload):** `functions/api/headshot.js` and `functions/api/headshot/[id].js` (the `headshot/` subfolder under `functions/api/` is new, so create it when uploading the nested `[id].js`). **NEW files (Batch 5, Official Documents):** `functions/api/docs.js` and `functions/api/docs/[id].js` (the `docs/` subfolder is new). **NEW files (Batch 7):** `src/components/ZeffyFrame.astro`, `functions/api/gallery.js`, and `functions/api/gallery/[id].js` (the `gallery/` subfolder under `functions/api/` is new, so create it when uploading the nested `[id].js`).
