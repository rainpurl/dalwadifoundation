// Pull staff-edited content from the API and patch the page. Baked HTML is the fallback,
// so the page is fully readable even if the API is unreachable.
(function(){
  "use strict";
  ...
  fetch('/api/content').then(function(r){ return r.ok ? r.json() : null; }).then(function(data){
    if (!data) return;
    // generic content paths (about/contribute)
    ...
    // a donate link / email on the contribute page
    ...
    // landing pillar text (existing pillars only; a brand-new pillar needs a redeploy)
